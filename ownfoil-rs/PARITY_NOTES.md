...
Upstream HEAD checked September 17: `84f0b352332cf4ac467db16228b10779f2eb2108`.
The only subsequent change is documentation; the feature baseline is unchanged.
...
- Realtime connections send 25-second heartbeat pings and stop when a peer fails
  to respond. Task labels include filenames and library paths, with fallbacks for
  removed files and stale task arguments.
...
- Conversion retains verification results while updating the output timestamp;
  subsequent external modifications still invalidate those results on rescan.
...
- Browser interaction checks remain open. Container CI passed for `b97b005`;
  local container runtime checks remain open because Docker was stopped.
